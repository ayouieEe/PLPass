...
-- Scheduled time remains the event's official reference.  Once an organizer
-- starts a live session, however, attendance must follow the recorded actual
-- start/end.  In particular, a delayed start must not be force-closed at the
-- scheduled end.
...
begin
  ...
  -- An ongoing session has no actual end.  Do not fall back to scheduled_end:
  -- a delayed session remains open until its organizer ends it.  The explicit
  -- window end is retained for any future controlled-close workflow.

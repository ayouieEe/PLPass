...
-- Keep the v2 checkout-method support without regressing the durable
-- unverified-walk-in path from the original sync function.
...
declare
  ...
  v_unverified public.unverified_walkin_attendance;
...
begin
  ...
  if v_matches <> 1 then
    select u.* into v_unverified from public.unverified_walkin_attendance u
    where u.local_scan_uuid = p_local_scan_uuid
    for update;
    if found then
      if v_unverified.event_id is distinct from p_event_id
         or v_unverified.event_session_id is distinct from p_session_id
         or upper(btrim(v_unverified.student_number)) is distinct from upper(btrim(p_student_number))
         or v_unverified.time_in is distinct from p_time_in
         or (v_unverified.time_out is not null and p_time_out is not null and v_unverified.time_out is distinct from p_time_out) then
        return jsonb_build_object('disposition', 'discarded_permanent_conflict', 'reasonCode', 'existing_scan_conflict');
      end if;
      if p_time_out is not null and v_unverified.time_out is null then
        update public.unverified_walkin_attendance
        set time_out = p_time_out,
            checkout_identification_method = v_checkout_method,
            updated_at = now()
        where id = v_unverified.id
        returning * into v_unverified;
      end if;
    else
      insert into public.unverified_walkin_attendance
        (local_scan_uuid, event_id, event_session_id, student_number, identification_method, checkout_identification_method, time_in, time_out, recorded_by)
      values
        (p_local_scan_uuid, v_event.id, v_session.id, btrim(p_student_number), p_identification_method,
         case when p_time_out is not null then v_checkout_method else null end, p_time_in, p_time_out, v_actor)
      returning * into v_unverified;
    end if;
    return jsonb_build_object('unverifiedWalkIn', jsonb_build_object(
      'id', v_unverified.id,
      'localScanUuid', v_unverified.local_scan_uuid,
      'studentNumber', v_unverified.student_number,
      'identificationMethod', v_unverified.identification_method,
      'checkoutIdentificationMethod', v_unverified.checkout_identification_method,
      'timeIn', v_unverified.time_in,
      'timeOut', v_unverified.time_out
    ));
  end if;

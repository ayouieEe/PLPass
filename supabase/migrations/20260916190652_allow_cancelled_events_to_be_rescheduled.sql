...
begin
  ...
  v_was_cancelled := v_event.event_status = 'cancelled';
  v_old_start := v_event.starts_at;
  v_old_end := v_event.ends_at;
  v_old_venue := v_event.venue;
  ...
  set venue = btrim(p_venue),
      ...
      ends_at = p_ends_at,
      event_status = case when v_was_cancelled then 'scheduled' else event_status end,
      cancellation_reason = case when v_was_cancelled then null else cancellation_reason end,
      cancelled_by = case when v_was_cancelled then null else cancelled_by end,
      cancelled_at = case when v_was_cancelled then null else cancelled_at end,
  ...
  values (
    v_actor,
    case when v_was_cancelled then 'event.reinstated_and_rescheduled' else 'event.rescheduled' end,
    'event',
    p_event_id,
    jsonb_build_object(
      'reason', btrim(p_reason),
      'old_start', v_old_start,
      'new_start', p_starts_at,
      'old_end', v_old_end,
      'new_end', p_ends_at,
      'old_venue', v_old_venue,
      'new_venue', p_venue,
      'restored_from_cancelled', v_was_cancelled
    )
  );
  return v_event;
end;

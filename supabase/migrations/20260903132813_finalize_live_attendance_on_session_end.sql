...
begin
  ...
  loop
    ...
    if v_status = 'late' and v_late_reason is null then raise exception 'A late reason is required.' using errcode = '22023'; end if;
    if v_time_out is not null and v_time_out < v_time_in then raise exception 'Checkout cannot be earlier than check-in.' using errcode = '22023'; end if;
    if not exists (

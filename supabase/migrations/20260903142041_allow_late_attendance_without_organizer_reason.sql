...
-- Late attendance may be finalized without an organizer-provided category.
-- The student supplies their explanation through the student attendance flow.
...
begin
  ...
  loop
    ...
    if v_time_out is not null and v_time_out < v_time_in then raise exception 'Time Out cannot be earlier than Time In.' using errcode = '22023'; end if;
    if not exists (

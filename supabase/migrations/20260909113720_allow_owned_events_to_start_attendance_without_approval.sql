...
-- Approval controls student publication. It is not an attendance-start gate for
-- an active organizer's own event. Ownership and terminal event states remain
-- enforced in both functions below.
...
begin
  ...
  if not found or v_event.organizer_id <> private.current_organizer_id() or v_event.event_status in ('completed', 'cancelled') then raise exception 'Only an owned active event can start attendance.' using errcode = '42501'; end if;

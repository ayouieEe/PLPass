...
-- Future offline walk-ins that cannot be reconciled to one active enrolled
-- student are explicitly rejected. Historic rows in
-- unverified_walkin_attendance are intentionally left untouched.
...
begin
  ...
  for update;
  if not found then
    return jsonb_build_object('disposition', 'discarded_permanent_conflict', 'reasonCode', 'event_not_owned');
  end if;
  ...
  if v_matches <> 1 then
    return jsonb_build_object('disposition', 'discarded_permanent_conflict', 'reasonCode', 'no_active_enrolled_student');
  end if;
  ...
  if exists (
    ...
    where ep.event_id = v_event.id and ep.student_id = v_student.id and ep.participant_status = 'removed'
  ) then
  ...
  if found then
    ...
    return jsonb_build_object(
      'disposition', 'confirmed_registered',
  ...
  values (
    v_actor, 'attendance.offline_walkin_reconciled', 'event_participant', v_event.id,

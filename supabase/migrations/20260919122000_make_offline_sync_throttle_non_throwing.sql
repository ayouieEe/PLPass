-- The first throttle used an exception for rate-limited calls. Every rejected
-- RPC therefore counted as an aborted transaction and could amplify a retry
-- storm. Return NULL instead: the client treats the missing confirmation as a
-- transient retry, while the database transaction can complete normally.

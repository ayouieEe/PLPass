...
begin
  ...
  from vault.decrypted_secrets
  where name = 'event_email_worker_url'
  limit 1;

  if v_service_role_key is null or v_worker_url is null then
    raise warning 'Event email worker is not configured: missing required Vault worker credentials.';
    return;
  end if;

...
export function OfflineStatusPanel({status,busy,autoSyncPaused,onPrepare,onRetry}:{status:OfflineStatus;busy:boolean;autoSyncPaused?:boolean;onPrepare:()=>void;onRetry:()=>void}){
  const online=status.connectivity==="online"; const total=status.pendingCount;
  const message=!status.runtimeAvailable?"Offline attendance is unavailable in browser":autoSyncPaused?`Automatic synchronization paused - ${total} record${total===1?"":"s"} retained locally`:!online?`Offline - ${total} records waiting to sync`:status.syncingCount?`Online - Synchronizing ${status.syncingCount} of ${total}`:status.conflictCount?`${status.conflictCount} conflict${status.conflictCount===1?"":"s"} requiring review`:total?`Online - ${total} records waiting to sync`:"All attendance synchronized";

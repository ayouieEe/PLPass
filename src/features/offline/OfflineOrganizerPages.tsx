...
import type { OfflinePreparedEventSummary, PendingAttendanceRecord, PreparedEventPackage } from "./types";
...
export function OfflineOrganizerLiveAttendancePage() {
  ...
  const capture = async () => {
    ...
    try {
      ...
      if (!student) throw new Error("No eligible student matched the saved package.");
      const result = await recordOfflineAttendance({ eventId:pkg.event.id,sessionId,studentId:student.studentId,identificationMethod:method,attendanceTimestamp:new Date().toISOString() },capturePhase);
      setStatus(`${student.displayName}: ${result.action === "checked_out" ? "Time Out saved" : result.action === "checked_in" ? "Time In saved" : "already recorded"} on this device; not yet synced.`);

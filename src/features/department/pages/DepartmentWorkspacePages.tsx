...
export function DepartmentBrandingPage() {
  ...
  const previewStyle = { "--department-preview-secondary": secondaryColor || "#e8f1e6" } as CSSProperties;
  ...
  return <div className="grid gap-5 xl:grid-cols-[minmax(0,1.2fr)_minmax(18rem,0.8fr)]">
    ...
    <aside className="space-y-4">
      <section className="overflow-hidden rounded-2xl border border-border bg-surface shadow-sm" aria-label="Live department branding preview">
        <div className="border-b border-border p-5"><p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Live preview</p><h2 className="mt-1 text-lg font-semibold">Department presentation</h2><p className="mt-1 text-sm text-muted-foreground">Preview updates as you edit; save to apply it.</p></div>
        <div className="bg-[var(--department-preview-secondary)] p-5 dark:bg-surface-muted" style={previewStyle}>
          <div className="flex items-center gap-3 rounded-xl border border-border bg-surface/90 p-4 shadow-sm">
            <div className="grid h-12 w-12 shrink-0 place-items-center overflow-hidden rounded-lg border border-border bg-background">{selectedLogoUrl ? <img src={selectedLogoUrl} alt="" className="h-full w-full object-contain" /> : <span className="text-sm font-bold" style={{ color: primaryColor || "#3f7a44" }}>{logoInitials}</span>}</div>
            <div className="min-w-0"><p className="truncate font-semibold" style={{ color: primaryColor || "#3f7a44" }}>{displayName || "Department name"}</p><p className="text-xs text-muted-foreground">Department workspace</p></div>
          </div>
          <div className="mt-3 h-2 rounded-full" style={{ backgroundColor: primaryColor || "#3f7a44" }} />
          <div className="mt-3 grid grid-cols-2 gap-2">
            {[0, 1].map((item) => <div key={item} className="rounded-lg border border-border bg-surface/90 p-3 shadow-sm"><span className="block h-2 w-12 rounded" style={{ backgroundColor: primaryColor || "#3f7a44" }} /><span className="mt-2 block h-2 w-16 rounded bg-muted" /></div>)}
          </div>
        </div>
      </section>
...
export function DepartmentSystemHealthPage() {
  ...
  return <div className="space-y-6"><PageHeader title="System Health" description="Check department data, failed event emails, and stale attendance sessions." actions={<button type="button" onClick={() => void refresh()} disabled={healthLoading || events.isFetching || attendance.isFetching || students.isFetching} className="inline-flex items-center rounded-xl border border-border bg-surface px-4 py-2.5 text-sm font-semibold disabled:opacity-50"><RefreshCw className={`mr-2 h-4 w-4 ${healthLoading || events.isFetching || attendance.isFetching || students.isFetching ? "animate-spin" : ""}`} />Refresh checks</button>} />
    ...
    {loading || healthLoading ? <LoadingState label="Loading department health…" /> : <>
      <section className="grid gap-4 md:grid-cols-3">{checks.map((check) => <div key={check.label} className={`rounded-2xl border p-5 ${check.healthy ? "border-success/30 bg-success-muted" : "border-warning/30 bg-warning-muted"}`}><div className="flex items-center justify-between gap-3"><h2 className="font-semibold text-foreground">{check.label}</h2>{check.healthy ? <CheckCircle2 className="h-5 w-5 text-success" /> : <AlertTriangle className="h-5 w-5 text-warning" />}</div><p className="mt-3 text-sm text-muted-foreground">{check.detail}</p></div>)}</section>

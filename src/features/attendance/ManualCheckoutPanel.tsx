...
const checkoutReasons = ["Early departure", "Medical issue", "Emergency", "Personal reason", "System issue", "Other"];

export function ManualCheckoutPanel({ studentId, reason, remarks, students, disabled, onStudentChange, onReasonChange, onRemarksChange, onSubmit }: ManualCheckoutPanelProps) {
  return (
    <section className="rounded-lg border bg-surface p-4" aria-label="Manual check-out entry">
      <h2 className="font-semibold">Manual check-out</h2>
      ...
      <div className="mt-4 space-y-3">
        ...
        <label className="block text-sm font-medium">
          ...
          <select className="plpass-field mt-1 h-10 w-full rounded-md border px-3 text-sm" value={reason} disabled={disabled} onChange={(event) => onReasonChange(event.target.value)}>
            ...
            {checkoutReasons.map((item) => <option key={item} value={item}>{item}</option>)}
          </select>
        </label>
        <label className="block text-sm font-medium">
          Remarks

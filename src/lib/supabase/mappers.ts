...
export function mapEvent(row: Row): Event {
  ...
  return {
    ...
    institutionalCategory: optionalString(row, ["institutional_category"]) as Event["institutionalCategory"],
    participationStatus: optionalString(row, ["participation_status"]) as Event["participationStatus"],
    targetGroup: optionalString(row, ["target_group"]) as Event["targetGroup"],
    urgencyPoints: nullableNumberValue(row, ["urgency_points"]) ?? 0,
...
export function mapAttendanceRecord(row: Row): AttendanceRecord {
  const base = {
    ...
    verificationMethod: (optionalString(row, ["verification_method"]) ?? null) as VerificationMethod | null,
    checkoutVerificationMethod: (optionalString(row, ["checkout_verification_method"]) ?? null) as VerificationMethod | null | undefined,

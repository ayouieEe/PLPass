-- Staging-only exact cleanup for lost-response-fixture.sql.
-- Run only after recording the required evidence.
do $$
declare
  v_event_id uuid;
  ...
  v_category_id uuid;
begin
  select id into v_event_id from public.events where event_code = 'STG-LOST-RESPONSE';
  select id into v_department_id from public.departments where department_code = 'STG-LR';
  select id into v_program_id from public.programs where department_id = v_department_id and program_code = 'STG-LR';
  select id into v_section_id from public.sections where program_id = v_program_id and section_name = 'LR' and academic_year = '2099-2100' and semester = 'Lost Response';
  select id into v_category_id from public.event_categories where category_name = 'Staging Lost Response';

  if v_event_id is not null then delete from public.events where id = v_event_id; end if;
  delete from auth.users where email in ('plpass-lost-response-organizer@staging.invalid', 'plpass-lost-response-student@staging.invalid');

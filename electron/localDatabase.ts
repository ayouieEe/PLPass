...
export class LocalAttendanceDatabase {
  ...
  prepareEvent(pkg: PreparedEventPackage, organizerProfileId = pkg.organizerProfileId ?? "test-organizer"): OfflineStatus {
    ...
    this.transaction(() => {
      ...
      const session = this.db.prepare(`INSERT INTO cached_sessions(session_id,event_id,title,venue,session_status,starts_at,ends_at,late_cutoff_at,attendance_window_start_at,attendance_window_end_at,offline_lifecycle,offline_start_reconciled_at)
        VALUES(@id,@eventId,@title,@venue,@status,@startsAt,@endsAt,@lateCutoffAt,@attendanceWindowStartAt,@attendanceWindowEndAt,@offlineLifecycle,@offlineStartReconciledAt)`);
      pkg.sessions.forEach((item) => session.run({ ...item, offlineLifecycle:item.status==="ongoing"?"STARTED":"NOT_STARTED", offlineStartReconciledAt:item.status==="ongoing"?(item.attendanceWindowStartAt ?? pkg.preparedAt):null, lateCutoffAt: item.lateCutoffAt ?? null, attendanceWindowStartAt: item.attendanceWindowStartAt ?? null, attendanceWindowEndAt: item.attendanceWindowEndAt ?? null }));
  ...
  setOfflineLifecycleState(eventId:string,sessionId:string,state:"STARTED"|"ENDED"|"CONFLICT") {
    if(state==="STARTED") this.db.prepare("UPDATE cached_sessions SET offline_start_reconciled_at=COALESCE(offline_start_reconciled_at,?),offline_lifecycle=CASE WHEN offline_ended_at IS NOT NULL THEN 'END_PENDING' ELSE 'STARTED' END WHERE session_id=? AND event_id=?").run(new Date().toISOString(),sessionId,eventId);
  ...
  private recordAttendanceForPhase(input: LocalAttendanceInput, phase: AttendanceCapturePhase): LocalAttendanceResult {
    ...
    const result = this.transaction(() => {
      ...
      if (!session || String(session.session_status) !== "ongoing" || !["START_PENDING","STARTED"].includes(String(session.offline_lifecycle))) throw new Error("No active prepared event session is available.");
  ...
  queueWalkInScan(input: {eventId:string;sessionId:string;studentNumber:string;identificationMethod:"qr"|"manual";capturePhase:AttendanceCapturePhase;attendanceTimestamp:string;organizerProfileId:string}): PendingWalkInScan {
    ...
    if(input.capturePhase==="time_in") {
      ...
      // retry is idempotent instead of falling through as a lost attendance.
      if(existing) return this.mapWalkIn(existing, "already_recorded");
      const id=randomUUID();
      ...
      return this.mapWalkIn(this.db.prepare("SELECT * FROM pending_walkin_scans WHERE local_scan_uuid=?").get(id) as SqlRow, "checked_in");
    }

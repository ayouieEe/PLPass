...
export class LocalAttendanceDatabase {
  ...
  recoverInterruptedSync(){ return this.db.prepare("UPDATE pending_attendance SET sync_status='RETRY',next_attempt_at=NULL,last_sync_error='Synchronization was interrupted and will be retried.',updated_at=? WHERE sync_status='SYNCING'").run(new Date().toISOString()).changes; }

...
async function generateMasterReport() {
  ...
  if (plpLogo) {
    const logoId = workbook.addImage({ base64: plpLogo, extension: "png" });
    worksheet.addImage(logoId, "A1:A3");
  }
  if (customLogo) {
    const logoId = workbook.addImage({ base64: customLogo, extension: "png" });
  ...
  fs.writeFileSync("master_report.xlsx", Buffer.from(buffer));
  console.log("Generated master_report.xlsx successfully!");
}
